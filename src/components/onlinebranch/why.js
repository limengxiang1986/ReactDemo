...
class Why extends PureComponent{
  render(){
    ...
    return (
        <>
            {this.renderContent(why, css, comments, hightrelationeleflag)}
        </>
    )
  }
  renderContent = (why,css,comments, hightrelationeleflag) =>{
    let eleid = why.eleid;
    let hightlightclass = (hightrelationeleflag ? "hightlight" : "" ) ;
    return <div className={"why " + hightlightclass} style={{height: css.multiple*(css.eleheight*why.rsize)+(why.rsize-1)*css.marginbottom+"px",
